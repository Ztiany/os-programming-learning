...
struct event_loop {
     ...
     *
     * 为什么步骤 2 必须运行在 event_loop 所绑定的线程中呢？因为与 event_dispatcher 进行交互的操作都运行在 event_loop 所绑定的线程中就可以不用加锁了，也就是说分发 event 和 处理 event
     * 的操作就都不用加锁了。
     */
...
int event_loop_add_channel_event(struct event_loop *event_loop, int fd, struct channel *channel);

/** 将需要从 event_dispatcher 中删除的 channel 添加到 event_loop 的队列中 */
int event_loop_remove_channel_event(struct event_loop *event_loop, int fd, struct channel *channel);

/** 将需要更新的 channel 添加到 event_loop 的队列中 */
int event_loop_update_channel_event(struct event_loop *event_loop, int fd, struct channel *channel);

/** 处理队列中需要添加到 event_dispatcher 的 channel */
int event_loop_handle_pending_add(struct event_loop *event_loop, int fd, struct channel *channel);

/** 处理队列中需要从 event_dispatcher 删除的 channel */
int event_loop_handle_pending_remove(struct event_loop *event_loop, int fd, struct channel *channel);

/** 处理队列中需要从 event_dispatcher 更新的 channel */
int event_loop_handle_pending_update(struct event_loop *event_loop, int fd, struct channel *channel);

/** 当 event_dispatcher 收集完需要处理的事件后，通过该方法将其分发到对应的 event_loop 进行处理，events 为单个事件或多个事件的组合 */
int event_loop_active(struct event_loop *event_loop, int fd, int events);
