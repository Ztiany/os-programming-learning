...
struct event_dispatcher {
    ...
    void (*add)(struct event_loop *event_loop, struct channel *channel);

    /** 将一个 channel 从 event_dispatcher 上删除 */
    void (*delete)(struct event_loop *event_loop, struct channel *channel);

    /** 更新已经在 event_dispatcher 上注册的 channel */
    void (*update)(struct event_loop *event_loop, struct channel *channel);

    /** 在该 event_dispatcher 上等待事件，并进行分发【比如调用 select 方法】*/
    void (*dispatch)(struct event_loop *event_loop, struct timeval *timeout);
